...
def get_dates(years=10):
    ...
    for i in range(365, 365 * years, 365):
...
"""
def get_symbols():
    res = engine_mysql.execute("SELECT symbol FROM sentosa.ticker_info where marketCap>2e8 and volume>2e5 order by symbol").fetchall()
    r = [i[0] for i in res]
    return r

def get_etf_symbols():
    res = engine_mysql.execute("SELECT symbol FROM sentosa.ticker_etf where oneYearPercentage>20 and lastSalePrice>0").fetchall()
    r = [i[0] for i in res]
    return r

equities = get_symbols()
etfs = get_etf_symbols()
#symbols = equities + etfs
symbols = etfs
...
def main():
    ...
    for symbol in symbols:
        ...
        for from_date, to_date in dates:
            sql = f"SELECT count(*) as cnt FROM sentosa.`bar1d_poly` where s='{symbol}' and dt>='{from_date}' and dt<'{to_date}'"
            res = engine_mysql.execute(sql).fetchall()
            print(res, from_date, to_date)
            if int(res[0][0]) > 200: #60*8*18:
                continue
                #break
            count = 0
            retry = 0
            with RESTClient(key) as client:
                try:
                    resp = client.stocks_equities_aggregates(symbol, 1, "day", from_date, to_date, unadjusted=False, limit=50000)
                    ...
                    for result in resp.results:
                        ...
                        if 'vw' not in result:
                            result['vw'] = result['o']
                        values.append(f"('{symbol}',{result['o']},{result['h']},{result['l']},{result['c']},{result['v']},{result['vw']},'{dt}')")
                        count += 1
                        if count % 500 == 2:
                            write_bar1d(engine_mysql, values)
                            values = []
...
def write_bar1d(engine, values):
    if values:
        vs = ',\n'.join(values)
        sql_dt = f"insert ignore into sentosa.`bar1d_poly`(s,o,h,l,c,v,w,dt)VALUES{vs}"
        #print(f"sql: {sql_dt}")
