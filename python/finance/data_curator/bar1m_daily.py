...
"""
if tm < '17-00-00':
    print("The program need to be ran after after-market hour!")
    sys.exit(0)
"""
...
def get_symbols():
    res = engine_mysql.execute(
        "SELECT symbol FROM sentosa.ticker_info where marketCap>2e8 and volume>2e5 order by symbol").fetchall()
    r = [i[0] for i in res]
    return r
...
def main():
    ...
    for symbol in symbols:
        for from_date, to_date in dates:
            ...
            print(symbol, res[0][0], from_date, to_date)
            if int(res[0][0]) > 100:  # 60*6.5:  The polygon data is not continuous!
                break
            count = 0
            retry = 0
            with RESTClient(key) as client:
                try:
                    resp = client.stocks_equities_aggregates(symbol, 1, "minute", from_date, to_date, unadjusted=False,
                                                             limit=50000)
                    ...
                    for result in resp.results:
                        ...
                        if 'vw' not in result:
                            result['vw'] = result['o']
                        values.append(
                            f"('{symbol}',{result['o']},{result['h']},{result['l']},{result['c']},{result['v']},{result['vw']},-1,'{dt}')")
                        count += 1
                        if count % 5000 == 2:
