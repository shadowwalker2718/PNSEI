...
index_summary_file_name = os.path.expanduser(f"~/index_summary_{ymd}.csv")
if os.path.exists(index_summary_file_name):
    print(f"file {index_summary_file_name} exists already")
else:
    ...
    total_records = 1111111111
    rows = []
    offset = len(rows)
    while total_records != len(rows):
        url = f'https://api.nasdaq.com/api/screener/index?tableonly=false&limit=100&offset={offset}'
        ...
        total_records = resp['data']['records']['totalrecords']
        offset = len(rows)

    with open(os.path.expanduser(f"~/index_tickers_{ymd}.csv"), '+w') as index_ticker_file:
        for row in rows:
            line = row['symbol'] + "\n"
            index_ticker_file.write(line)

    with open(index_summary_file_name, '+w') as index_summary_file:
        line = '"' + '","'.join(rows[0].keys()) + '"\n'
        index_summary_file.write(line)
        for row in rows:
            ...
            if 'deltaIndicator' in row and row['deltaIndicator'] == 'down':
                row['netChange'] = row['netChange'] * -1
                row['percentageChange'] = row['percentageChange'] * -1
            line = '"' + '","'.join(str(item) for item in row.values()) + '"\n'
            index_summary_file.write(line)

    print(f"Wrote {len(rows)} lines into file {index_summary_file_name}")

##############################################################################################3
df = pd.read_csv(index_summary_file_name, dtype={'percentageChange': float, 'priceChange': float})
...
with engine_mysql.begin() as cx:
    ...
    df.to_sql('ticker_index', con=cx, dtype=data_type, index=True,
