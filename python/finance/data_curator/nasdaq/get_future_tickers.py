...
url = 'https://api.nasdaq.com/api/screener/futures?tableonly=true&limit=10000&offset=0&download=true'

summary_file_name = os.path.expanduser(f"~/future_summary_{ymd}.csv")
if os.path.exists(summary_file_name):
    print(f"file {summary_file_name} exists already")
    sys.exit(0)
...
with open(os.path.expanduser(f"~/future_tickers_{ymd}.csv"), '+w') as future_ticker_file:
    for row in rows:
        line = row['symbol'] + "\n"
        future_ticker_file.write(line)

with open(summary_file_name, '+w') as future_summary_file:
    line = '"' + '","'.join(rows[0].keys()) + '"\n'
    future_summary_file.write(line)
    for row in rows:
...
print(f"Wrote {len(rows)} lines into file {summary_file_name}")


##############################################################################################3
df = pd.read_csv(summary_file_name, dtype={'percentageChange': float, 'priceChange': float})
...
with engine_mysql.begin() as cx:
    ...
    df.to_sql('ticker_future', con=cx, dtype=data_type, index=True,
