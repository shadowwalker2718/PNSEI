...
etf_summary_file_name = os.path.expanduser(f"~/etf_summary_{ymd}.csv")
...
if os.path.exists(etf_summary_file_name):
    print(f"file {etf_summary_file_name} exists already")
else:
    ...
    with open(os.path.expanduser(f"~/etf_tickers_{ymd}.csv"), '+w') as etf_ticker_file:
        for row in rows:
            line = row['symbol'] + "\n"
            etf_ticker_file.write(line)

    with open(etf_summary_file_name, '+w') as etf_summary_file:
        line = '"' + '","'.join(rows[0].keys()) + '"\n'
        etf_summary_file.write(line)
        for row in rows:
            ...
            process_row_percent(row, 'oneYearPercentage')
    ...
    print(f"Wrote {len(rows)} lines into file {etf_summary_file_name}")



##############################################################################################3
df = pd.read_csv(etf_summary_file_name, dtype = {'oneYearPercentage': float, 'netChange': float})
...
with engine_mysql.begin() as cx:
    ...
    for idx, row in df.iterrows():
        s = idx
        sql = f"select * from ticker_etf where symbol='{s}'"
        d = pd.read_sql(sql, cx)
        if d.shape[0] == 1:
            # compare d and row
            for i in ['lastSalePrice', 'priceChange', 'percentageChange', 'updated', 'oneYearPercentage']:
                d[i] = df.loc[s][i]
        ...
        d.to_sql('ticker_etf', con=cx, dtype=data_type, index=True,
