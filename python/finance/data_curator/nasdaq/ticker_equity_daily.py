...
with engine_mysql.begin() as cx:
    ...
    for idx, row in df.iterrows():
        s = idx
        sql = f"select * from ticker_equity where symbol='{s}'"
        d = pd.read_sql(sql, cx)
        if d.shape[0] == 1:
            # compare d and row
            for i in ['lastsale', 'netchange', 'pctchange', 'volume', 'marketCap', 'updated']:
                d[i] = df.loc[s][i]
        ...
        d.to_sql('ticker_equity', con=cx, dtype=data_type, index=True, if_exists='append', index_label='symbol', method=my_method)
