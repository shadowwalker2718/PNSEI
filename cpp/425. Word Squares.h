...
public:
  ...
  void f(vector<vector<string>> &r, vector<string> &p, string &s, trie *ptr,
         int start) {
    p.push_back(s);
    if (start >= s.size())
      r.push_back(p);
...
struct Solution3 { // 75ms, 90%
  ...
  vector<vector<string>> wordSquares(vector<string> &words) {
    ...
    vector<string> p;
    for (string &s : words)
      dfs(r, p, trie, s.c_str(), 1);
    return r;
  }
  void dfs(vector<vector<string>> &r, vector<string> &p, vector<TrieNode> &trie,
           const char *s, int k) {
    p.push_back(s);
    if (k == strlen(s)) {
      r.push_back(p);
      ...
      int i = 0;
      for (string &ns : p)
        if ((i = trie[i].nodes[ns[k] - 'a']) == 0)
          break;
      if (i > 0)
        for (const char *ns : trie[i].words)
          dfs(r, p, trie, ns, k + 1);
    }
    p.pop_back();
  }
