...
NTNode * build_tree(vector<string> vs){
  map<int, vector<NTNode *>> m;
  for(string s: vs){
    if (s.rfind(' ')==s.npos){
      m[0].push_back(new NTNode);
      m[0].back()->key =s.back();
    }else{
      int t=s.rfind(' ');
...
NTNode * build_tree2(vector<string> vs){
  vector<pair<int, NTNode *>> m; // {length, node*}
  int i=0;
  for(string s: vs){
    if (s.rfind(' ')==s.npos){
      m.emplace_back(0, new NTNode);
