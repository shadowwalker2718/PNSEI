...
被技术问题问倒了,多纬度数据如何存储,查找,这个不难kd-graph_MST,然后区间查找如何o(1)这个真不知道,
 然后问了些神经网络的东西,我做
 过一些,但从没研究过神经网络里面模型的意义,被问到了自己调的网络的模型意义,半年前搭的网络早忘了具体
 模型意义,答不上来,然后问了
 如何在实际中应用,没有实际相关领域工作经验答不上来...
...
如GLR, 或是其他的如LR,
给定的操作直行无限次操作,请问哪种操作序列可以保证robot会停留在一个圆里面活动（即不会一直往前走)
第三题是hackrank上的clique题,你可以去看看
