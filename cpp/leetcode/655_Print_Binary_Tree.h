...
namespace lc_655{
  ...
  struct Solution {
    ...
    void dfs(TreeNode* n, int j, vector<vector<string>>& res, int i, int C){ // C=7
      if(!n) return;
  ...
  //Algo: find the depth, BFS, put null into queue, null's children are nulls, which we also put into queue
