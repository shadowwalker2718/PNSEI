...
public:
  bool isRectangleCover(vector<vector<int>> rectangles) {
    ...
    if (sp.size() != 4)
      return false;
    pair<int, int> p0 = *sp.begin(),
                   p1 = *sp.rbegin(); // , p2 = { p0.first, p1.second }, p3 = {
                                      // p1.first, p0.second };
    return (p1.second - p0.second) * (p1.first - p0.first) ==
           area; // sp.count(p2) && sp.count(p3)
  }
};
...
public:
  bool isRectangleCover(vector<vector<int>> rectangles) {
    ...
    return 4 == sp.size() && sp.count(mi) && sp.count(mx) &&
...
public:
  bool isRectangleCover(vector<vector<int>> rectangles) {
    ...
    for (auto val : rectangles) {
      for (int i = 0; i < 4; i++) {
        string tem =
            to_string(val[i / 2 * 2]) + ',' + to_string(val[i % 2 * 2 + 1]);
        if (hash[tem] & (1 << i))
          return false;
        hash[tem] |= (1 << i);
      }
    ...
    for (auto &val : hash) {
      ...
      if (!(sec & (sec - 1)) && cntCorner++ > 4)
        return false;
      if ((sec & (sec - 1)) &&
          !(sec == 3 || sec == 12 || sec == 5 || sec == 10 || sec == 15))
        return false;
    }
    return true;
  }
...
public:
  bool isRectangleCover(vector<vector<int>> &rectangles) {
    ...
    for (auto &v : rectangles) {
      area += (v[2] - v[0]) * (v[3] - v[1]);
      tmp = ((long long)(v[0]) << 32) | (v[1] & mask) | ((v[0] & SIGN) << 32) |
            ((v[1] & SIGN) << 32);
      mi = min(tmp, mi);
      if (sp.count(tmp))
        sp.erase(tmp);
      ...
      if (sp.count(tmp))
        ...
        sp.insert(tmp);
      tmp = ((long long)(v[2]) << 32) | (v[3] & mask) | ((v[2] & SIGN) << 32) |
            ((v[3] & SIGN) << 32);
      mx = max(mx, tmp);
      if (sp.count(tmp))
        sp.erase(tmp);
    ...
    cout << mx << endl;
    cout << mi << endl;
    cout << (((mx & (~(1LL << 63))) >> 32) - ((mi & (~(1LL << 63))) >> 32))
         << endl;
    cout << (int)(mx & mask) << endl;
    cout << (int)(mi & mask) << endl;
    cout << (((mx & LONG_MAX) >> 32) - (((mi & LONG_MAX) >> 32))) *
                ((int)(mx & mask) - (int)(mi & mask))
         << endl;
    return 4 == sp.size() && sp.count(mi) && sp.count(mx) &&
           (((mx & (~(1LL << 63))) >> 32) - ((mi & (~(1LL << 63))) >> 32)) *
                   ((int)(mx & mask) - (int)(mi & mask)) ==
               area;
  }
};
...
void test() {
  ...
  assert(s.isRectangleCover(
         ...
         false);
  assert(s.isRectangleCover({{0, 0, 4, 1}, {0, 0, 4, 1}}) == false);
